...
- **Local-move archives only.** `BotArchiver`'s S3 mode tars and `rmtree`s the source (`bot_archiver.py:28-36`) and the repo has no download/extract path — S3-archived state is out of scope unless an S3 restore is added later.
- **Handle the nesting pathology.** `bot_archiver` has no same-name collision handling (`:45-53`): archiving a name twice nests `bots/archived/<name>/<name>`. Resolution must detect nested dirs and either resolve the innermost complete instance or refuse with a clear error.
...
Every failure aborts **before** the container launches: a failed resume can never produce a silently-fresh, mis-seeded live bot.
